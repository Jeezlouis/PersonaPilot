...
let jobsState = {
  ...
  page: 0,
  limit: 30,
  filters: { status: '', role_category: '', job_type: '', search: '' },
};
...
async function skipJob(jobId) {
  try {
    ...
    showToast('Job skipped', 'info', 2000);
  } catch (e) {
    ...
    content.innerHTML = `
      <div class="job-detail-panel" style="border:none;padding:0;">
        ...
        <div style="margin-bottom:24px;">
          <button class="btn btn-ghost btn-sm" onclick="document.getElementById('raw-data-${jobId}').style.display = 'block'; this.style.display='none';">📋 View Raw Job Scrape Data</button>
          <pre id="raw-data-${jobId}" style="display:none;font-size:11px;background:#0c0d12;padding:12px;border-radius:4px;overflow-x:auto;color:var(--text-muted);border:1px solid rgba(255,255,255,0.05);">${escHtml(JSON.stringify(job.raw_data, null, 2))}</pre>
        </div>
        ` : ''}
...
document.getElementById('filter-clear').addEventListener('click', () => {
  jobsState.filters = { status: '', role_category: '', job_type: '', search: '' };
